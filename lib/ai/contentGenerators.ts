 ...
 * Generates game content using OpenAI API with Christmas-themed prompts
 */
 ...
 */
export async function generateAITriviaQuestions(count: number = 10): Promise<TriviaItem[]> {
  try {
    const prompt = AI_PROMPTS.trivia(count);
...
/**
 * Generate Family Feud question using AI
 */
export async function generateAIFamilyFeudQuestion(): Promise<FamilyFeudQuestion> {
  try {
    const prompt = AI_PROMPTS.familyFeud();
    const response = await callOpenAI(prompt);
    const parsed = parseJSONResponse(response);
...
/**
 * Generate Pictionary prompts using AI
 */
export async function generateAIPictionaryPrompts(count: number = 20): Promise<PictionaryItem[]> {
  try {
    const prompt = AI_PROMPTS.pictionary(count);
    const response = await callOpenAI(prompt);
    const parsed = parseJSONResponse(response);
...
/**
 * Generate Would You Rather questions using AI
 */
export async function generateAIWouldYouRather(count: number = 10): Promise<WouldYouRatherItem[]> {
  try {
    const prompt = AI_PROMPTS.wouldYouRather(count);
    const response = await callOpenAI(prompt);
    const parsed = parseJSONResponse(response);
...
/**
 * Generate Emoji Movies using AI
 */
export async function generateAIEmojiMovies(count: number = 15): Promise<EmojiItem[]> {
  try {
    const prompt = AI_PROMPTS.emojiMovies(count);
    const response = await callOpenAI(prompt);
    const parsed = parseJSONResponse(response);
